...
const totpRemainingSecondsCircle = new ProgressBar.Circle(
  document.getElementById('totp-token-remaining-seconds-circle'),
  {
    strokeWidth: 50,
    duration: 1000,
...
document.getElementById('inputSecret').addEventListener(
  'input',
  () => {
    parseSecretInput()
    updateTotpGenerator()
...
document.getElementById('inputAccount').addEventListener(
  'input',
  () => {
    updateLabel()
    updateQrCode()
...
document.getElementById('inputIssuer').addEventListener(
  'input',
  () => {
    updateLabel()
    updateQrCode()
...
function refreshTotpToken() {
  ...
  if (totpGenerator) {
    try {
      totpTokenElement.innerHTML = totpGenerator
        .getToken()
        .replace(/(...)(...)/g, '<span>$1</span><span style="margin-left:8px">$2</span>')
      const normalizedRemainingTime =
        totpGenerator.getRemainingSeconds() / totpGenerator.getStepSeconds()
    ...
    totpTokenElement.innerHTML = '000000'.replace(
      /(...)(...)/g,
      '<span>$1</span><span style="margin-left:8px">$2</span>'
    )
    totpRemainingSecondsCircle.set(0.0)
  }
}
